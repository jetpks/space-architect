...
- `docs/gates/ui-interactive.md` — CLI-UX Slice B (color + fiber-driven live
  ...
  real-TTY smoke). **BUILT + POST-FLIGHT PASS + PRESERVED `2eab644` → dep-drop
  `362829a`; GATES PENDING (rule 4).** Builder: Sonnet 4.6 via `claude -p`.
...
- `docs/gates/ui-interactive-compact.md` — **Slice B CORRECTIVE (compact display)**,
  ...
  separate slice (reactor-yield), NOT a Slice-B hack. **BUILT + POST-FLIGHT PASS +
  PRESERVED `fde93e2`; GATES PENDING (rule 4).** Builder (Sonnet 4.6, 57 turns,
...
- **Slice `sync-startup` (responsive org expansion) — SPEC'D + FROZEN + DISPATCHED
  ...
  `.architect/sync-startup-01.block.md`. **A fresh session post-flights → judges
  GS0–GS7 + carried (GC1–GC3, G1/G4/G5, Slice-2 G10, Slice-4 CF3) + cross-model
  pass → human M1 (responsiveness + listing display + sweep) → merge `--no-ff`.**
