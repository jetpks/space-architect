  ...
  module Actions
    module Annotations
      class Create < Architect::Action
        include Architect::Deps["repos.conversations_repo", "repos.annotations_repo"]
        ...
        def handle(req, res)
          ...
          if (kind = ann_attrs[:target_kind])
            turns  = Architect::Transcript::Turn.group(conversation.messages)
            entity = Architect::Transcript::Entity.locate(
              turns:             turns,
              kind:              kind,
