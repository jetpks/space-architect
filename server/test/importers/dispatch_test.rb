...
class ImportersDispatchTest < Minitest::Test
  ...
  def test_selects_claude_code_for_transcript
    ...
    assert_equal Architect::Importers::ClaudeCode, Architect::Importers.select(record)
  end

  def test_selects_codex_for_codex_rollout
    record = fixture_first_record("codex_rollout.jsonl")
    assert_equal Architect::Importers::Codex, Architect::Importers.select(record)
  end

  def test_selects_pi_for_pi_session
    record = fixture_first_record("pi_session.jsonl")
    assert_equal Architect::Importers::Pi, Architect::Importers.select(record)
  end
  ...
  def test_nil_record_falls_back_to_claude_code
    assert_equal Architect::Importers::ClaudeCode, Architect::Importers.select(nil)
  end

  def test_unparseable_record_falls_back_to_claude_code
    assert_equal Architect::Importers::ClaudeCode, Architect::Importers.select("not a hash")
    assert_equal Architect::Importers::ClaudeCode, Architect::Importers.select(42)
  end

  def test_dispatcher_does_not_instantiate_importers
    # Verify select returns a Class, not an instance.
    result = Architect::Importers.select(nil)
