...
require "stringio"
require "space/server/runs/ingest"
...
class IngestTest < Minitest::Test
  ...
  def test_events_appear_in_stream
    ...
    with_redis do |redis|
      ...
      redis.del(key)
      result = Space::Server::Runs::Ingest.new(redis).call(run, StringIO.new(FIXTURE))
  ...
  def test_ttl_is_set_on_stream_key
    ...
    with_redis do |redis|
      key = Space::Server::Runs::StreamKey.for(run.id)
      redis.del(key)
      Space::Server::Runs::Ingest.new(redis).call(run, StringIO.new(FIXTURE))
      ttl = redis.ttl(key)
  ...
  def test_empty_input_returns_zero_events_without_crash
    run = Factory[:run, user_id: @user.id, status: 0]
    with_redis do |redis|
      result = Space::Server::Runs::Ingest.new(redis).call(run, StringIO.new(""))
  ...
  def test_stream_entries_have_type_and_data_fields
    ...
    with_redis do |redis|
      key = Space::Server::Runs::StreamKey.for(run.id)
      redis.del(key)
      Space::Server::Runs::Ingest.new(redis).call(run, StringIO.new(MINIMAL_JSONL))
      entries = redis.xrange(key, "-", "+")
  ...
  def test_raising_io_leaves_terminal_run_complete_in_stream
    ...
    with_redis do |redis|
      ...
      result = Space::Server::Runs::Ingest.new(redis).call(run, io)
