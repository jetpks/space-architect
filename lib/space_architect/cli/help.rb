...
module SpaceArchitect
  module CLI
    ...
    # help" shown by `architect`, `space`, and every bare namespace (`space repo`,
    ...
    module Help
      ...
      # The richer header only makes sense at the true root (`architect`), not on
      # every sub-namespace listing.
      def header(result, pastel)
        return unless result.names.empty?

        "#{pastel.bold.cyan("space-architect")} #{pastel.dim(SpaceArchitect::VERSION)} " \
          "#{pastel.dim("— #{TAGLINE}")}\n"
      end
      ...
      # "architect" at the root, "space" / "space repo" inside a namespace. The
      # `space` binary injects "space" into ARGV, so $PROGRAM_NAME and the leading
      # namespace segment collide ("space space ..."); drop the duplicate.
...
# internals — see notes/ruby-cli-gems-report.md and [[evergreen-checkouts]].
module Dry
  class CLI
    module Usage
      def self.call(result)
        SpaceArchitect::CLI::Help.call(result)
      end
    end
  end
end
