    ...
    module Shell
      ...
      class Complete < Dry::CLI::Command
        ...
        def completion_candidates(kind, args)
          ...
          when "config-keys"     then SpaceCadet::Config::EDITABLE_KEYS
          ...
          when "fish-subcommands"   then %w[install uninstall path]
            ...
            raise SpaceCadet::Error, "Usage: space shell complete #{completion_kinds.join('|')}"
          end
        end

        def completion_values_for_config_key(key)
          case key
          when "git_clone_protocol" then SpaceCadet::Config::VALID_GIT_CLONE_PROTOCOLS
...
SpaceCadet::CLI::Registry.register "shell" do |prefix|
  prefix.register "init",     SpaceCadet::CLI::Shell::ShellInit
  prefix.register "fish",     SpaceCadet::CLI::Shell::Fish
  prefix.register "complete", SpaceCadet::CLI::Shell::Complete
end
