...
class RunCreatorTest < SpaceArchitectTest
  ...
  def test_create_posts_to_runs_with_auth_and_returns_id
    ...
    Sync do
      ...
      creator = SpaceArchitect::RunCreator.new("http://localhost", "secret-token", client: client)
      id      = creator.create
  ...
  # (b) Non-201 response raises SpaceArchitect::Error.
  def test_create_raises_on_non_201_response
    ...
    Sync do
      ...
      client  = Async::HTTP::Client.new(mock_endpoint)
      creator = SpaceArchitect::RunCreator.new("http://localhost", "bad-token", client: client)

      assert_raises(SpaceArchitect::Error) { creator.create }
  ...
  # (c-i) 201 with no id field raises SpaceArchitect::Error.
  def test_create_raises_on_missing_id_in_201_body
    ...
    Sync do
      ...
      client  = Async::HTTP::Client.new(mock_endpoint)
      creator = SpaceArchitect::RunCreator.new("http://localhost", "tok", client: client)
  ...
  # (c-ii) 201 with non-integer id raises SpaceArchitect::Error.
