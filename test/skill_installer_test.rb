...
class SkillInstallerTest < SpaceArchitectTest
  def setup
    @tmp = temp_env
  end
  ...
  def test_dest_root_claude_global
    path = SpaceArchitect::SkillInstaller.dest_root("claude", project: false, env: env)
    assert_equal File.join(@tmp[:root], "home", ".claude", "skills"), path.to_s
  end

  def test_dest_root_codex_global
    path = SpaceArchitect::SkillInstaller.dest_root("codex", project: false, env: env)
    assert_equal File.join(@tmp[:root], "home", ".agents", "skills"), path.to_s
  end

  def test_dest_root_opencode_global
    path = SpaceArchitect::SkillInstaller.dest_root("opencode", project: false, env: env)
    assert_equal File.join(@tmp[:root], "xdg-config", "skills"), path.to_s
  end

  def test_dest_root_pi_global
    path = SpaceArchitect::SkillInstaller.dest_root("pi", project: false, env: env)
  ...
  def test_dest_root_pi_global_respects_env_override
    ...
    path = SpaceArchitect::SkillInstaller.dest_root("pi", project: false, env: e)
  ...
  def test_dest_root_claude_project
    path = SpaceArchitect::SkillInstaller.dest_root("claude", project: true, env: env, cwd: @tmp[:root])
    assert_equal File.join(@tmp[:root], ".claude", "skills"), path.to_s
  end

  def test_dest_root_codex_project
    path = SpaceArchitect::SkillInstaller.dest_root("codex", project: true, env: env, cwd: @tmp[:root])
    assert_equal File.join(@tmp[:root], ".agents", "skills"), path.to_s
  end

  def test_dest_root_opencode_project
    path = SpaceArchitect::SkillInstaller.dest_root("opencode", project: true, env: env, cwd: @tmp[:root])
    assert_equal File.join(@tmp[:root], ".opencode", "skills"), path.to_s
  end

  def test_dest_root_pi_project
    path = SpaceArchitect::SkillInstaller.dest_root("pi", project: true, env: env, cwd: @tmp[:root])
  ...
  def test_dest_root_unknown_provider_raises
    assert_raises(SpaceArchitect::Error) do
      SpaceArchitect::SkillInstaller.dest_root("unknown", project: false, env: env)
    end
  end
  ...
  def test_install_copies_all_skills
    result = SpaceArchitect::SkillInstaller.install("claude", project: false, force: false, env: env)
  ...
  def test_install_unchanged_when_same_content
    SpaceArchitect::SkillInstaller.install("claude", project: false, force: false, env: env)
    result = SpaceArchitect::SkillInstaller.install("claude", project: false, force: false, env: env)
  ...
  def test_install_raises_when_dest_differs_without_force
    SpaceArchitect::SkillInstaller.install("claude", project: false, force: false, env: env)
    dest = SpaceArchitect::SkillInstaller.dest_root("claude", project: false, env: env).join("architect", "SKILL.md")
    dest.write("# tampered\n")
  ...
  def test_install_overwrites_with_force
    ...
    result = SpaceArchitect::SkillInstaller.install("claude", project: false, force: true, env: env)
  ...
  def test_install_unknown_provider_raises
    assert_raises(SpaceArchitect::Error) do
      SpaceArchitect::SkillInstaller.install("unknown", project: false, force: false, env: env)
    end
  end
  ...
  def test_dry_run_does_not_write_files
    result = SpaceArchitect::SkillInstaller.install("claude", project: false, force: false,
  ...
  def test_dry_run_reports_would_update_with_force
    ...
    result = SpaceArchitect::SkillInstaller.install("claude", project: false, force: true,
                                                     env: env, dry_run: true)
    architect = result[:skills].find { |s| s[:name] == "architect" }
