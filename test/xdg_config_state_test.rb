...
class XDGConfigStateTest < SpaceArchitectTest
  ...
  def test_config_expands_spaces_dir_against_env_home
    setup = temp_env
    env = setup.fetch(:env)
    config = Space::Core::Config.new(env: env, data: {})
  ...
  def test_paths_derive_from_base_dir
    env = {"HOME" => "/tmp/fh"}
    default_config = Space::Core::Config.new(env: env, data: {})
    ...
    override_config = Space::Core::Config.new(env: env, data: {"spaces_dir" => "~/custom-spaces"})
    assert_equal "/tmp/fh/custom-spaces", override_config.spaces_dir.to_s

    disabled_config = Space::Core::Config.new(env: env, data: {"src_dir" => ""})
  ...
  def test_state_tracks_current_and_recent_spaces
    ...
    loaded = Space::Core::State.load(env: env)
